...
class _AdminAppointmentsPageState extends State<AdminAppointmentsPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadAppointments() async {
    ...
    try {
      ...
      if (_clientSearchQuery.isNotEmpty) _applyClientFilter();
    ...
    if (currentTabIndex == 0) {
      ...
      if (_isRangeMode && _rangeStart != null && _rangeEnd != null) {
        filtered = filtered.where((app) {
          ...
          return !appDate.isBefore(_rangeStart!) && !appDate.isAfter(_rangeEnd!);
        }).toList();
      } else {
  ...
  void _applyClientFilter() {
    ...
    final filtered = _appointments.where((app) {
      ...
      return clientName.contains(query) ||
          (app['USERS']['telefono']?.toString().contains(query) ?? false) ||
          (app['USERS']['email']?.toString().toLowerCase().contains(query) ?? false);
    }).toList();
    setState(() => _clientAppointments = filtered);
  }
                      ...
                      children: [
                        Expanded(
                          child: OutlinedButton.icon(
                            onPressed: () async {
                              setDialogState(() {
                                _editSelectedDate = _editSelectedDate.subtract(const Duration(days: 1));
                              });
                              await loadTimeSlotsForCurrentStylist();
                            },
                            icon: const Icon(Icons.chevron_left, size: 16),
                            label: const Text('- 1 giorno', style: TextStyle(fontSize: 12)),
                            style: OutlinedButton.styleFrom(foregroundColor: Colors.white70, side: BorderSide(color: Colors.grey[600]!)),
                          ),
                        ),
                        const SizedBox(width: 8),
                        Expanded(
                          child: OutlinedButton.icon(
                            onPressed: () async {
                              setDialogState(() {
                                _editSelectedDate = _editSelectedDate.add(const Duration(days: 1));
                              });
                              await loadTimeSlotsForCurrentStylist();
                            },
                      ...
                      Row(
                        children: [
                          Expanded(
                            child: OutlinedButton.icon(
                              onPressed: () {
                                final current = _editTimeController.text;
                                if (current.isEmpty) return;
                                final parts = current.split(':');
                                final totalMin = int.parse(parts[0]) * 60 + int.parse(parts[1]) - 5;
                                if (totalMin < 0) return;
                                setDialogState(() {
                                  _editTimeController.text = '${(totalMin ~/ 60).toString().padLeft(2, '0')}:${(totalMin % 60).toString().padLeft(2, '0')}';
                                });
                              },
      ...
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: const [
            ...
            Tab(icon: Icon(Icons.person_search), text: 'Clienti'),
          ],
        ),
      ),
      body: SafeArea(
        child: Column(
          children: [
            _tabController.index == 2 ? _buildClientSearchSection() : _buildFiltersSection(),
            Expanded(
              child: _isLoading
      ...
      child: Column(
        children: [
          ...
          if (_tabController.index == 0)
            // TAB OGGI: mostra la data selezionata in bianco grande
            InkWell(
              onTap: () async {
                final date = await showDatePicker(
                  context: context,
                  ...
                  builder: (context, child) => Theme(
                    data: Theme.of(context).copyWith(colorScheme: const ColorScheme.dark(primary: Colors.red, onPrimary: Colors.white, surface: Color(0xFF2d2d2d), onSurface: Colors.white)),
                    child: child!,
                  ),
                );
              ...
              child: Container(
                ...
                decoration: BoxDecoration(color: const Color(0xFF1a1a1a), borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.grey[700]!)),
                child: Row(
                  children: [
            ...
            // TAB PER DATA: toggle giorno/intervallo
            Row(
              children: [
                Expanded(
                  child: GestureDetector(
                    onTap: () { setState(() { _isRangeMode = false; }); _applyFilters(); },
                    child: Container(
                      padding: const EdgeInsets.symmetric(vertical: 8),
                      decoration: BoxDecoration(
                        color: !_isRangeMode ? Colors.red : const Color(0xFF1a1a1a),
                        borderRadius: const BorderRadius.horizontal(left: Radius.circular(8)),
                        border: Border.all(color: !_isRangeMode ? Colors.red : Colors.grey[700]!),
                      ),
                      child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                        Icon(Icons.today, size: 16, color: !_isRangeMode ? Colors.white : Colors.grey[400]),
                        const SizedBox(width: 6),
                        Text('Giorno', style: TextStyle(color: !_isRangeMode ? Colors.white : Colors.grey[400], fontSize: 13, fontWeight: FontWeight.w600)),
                      ]),
                    ),
                  ),
                ),
                Expanded(
                  child: GestureDetector(
                    onTap: () { setState(() { _isRangeMode = true; }); _applyFilters(); },
                    child: Container(
                      padding: const EdgeInsets.symmetric(vertical: 8),
                      decoration: BoxDecoration(
                        color: _isRangeMode ? Colors.red : const Color(0xFF1a1a1a),
                      ...
                      child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                        Icon(Icons.date_range, size: 16, color: _isRangeMode ? Colors.white : Colors.grey[400]),
                        const SizedBox(width: 6),
                        Text('Intervallo', style: TextStyle(color: _isRangeMode ? Colors.white : Colors.grey[400], fontSize: 13, fontWeight: FontWeight.w600)),
                      ]),
                    ),
                  ),
                ),
            ...
            if (!_isRangeMode)
              ...
              InkWell(
                onTap: () async {
                  ...
                  if (date != null) _onCustomDateChanged(date);
                },
                borderRadius: BorderRadius.circular(8),
                child: Container(
                  width: double.infinity,
              ...
              InkWell(
                onTap: () async {
                  ...
                  if (range != null) {
                    setState(() { _rangeStart = range.start; _rangeEnd = range.end; });
                    _applyFilters();
                  }
                },
                borderRadius: BorderRadius.circular(8),
                child: Container(
                  ...
                  child: Row(
                    children: [
                      Icon(Icons.date_range, color: Colors.grey[400], size: 18),
                      const SizedBox(width: 10),
                      Expanded(
                        child: Text(
                          (_rangeStart != null && _rangeEnd != null)
                              ? '${_formatDate(_rangeStart!.toString().split(' ')[0])}  →  ${_formatDate(_rangeEnd!.toString().split(' ')[0])}'
                              : 'Seleziona intervallo di date',
                          style: TextStyle(
                            color: (_rangeStart != null) ? Colors.white : Colors.grey[500],
                            fontSize: (_rangeStart != null) ? 15 : 14,
                            fontWeight: (_rangeStart != null) ? FontWeight.w600 : FontWeight.normal,
                          ),
                        ),
                      ),
                      Icon(Icons.edit_calendar, color: Colors.grey[500], size: 18),
      ...
      child: Column(
        children: [
          TextField(
            ...
            style: const TextStyle(color: Colors.white),
            decoration: InputDecoration(
              hintText: 'Cerca per nome, cognome o telefono...',
              hintStyle: TextStyle(color: Colors.grey[400]),
          ...
          if (_clientAppointments.isNotEmpty) ...[
            const SizedBox(height: 8),
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text('${_clientAppointments.length} appuntamento/i', style: TextStyle(color: Colors.grey[400], fontSize: 12)),
                Text(
                  'Totale: €${_clientAppointments.fold<double>(0, (sum, app) => sum + (app['prezzo_totale'] ?? 0)).toStringAsFixed(2)}',
  ...
  Widget _buildClientAppointmentsList() {
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text('Cerca un cliente per nome o telefono', style: TextStyle(color: Colors.grey[400], fontSize: 16), textAlign: TextAlign.center),
            const SizedBox(height: 8),
            Text('Verranno mostrati tutti i suoi appuntamenti', style: TextStyle(color: Colors.grey[600], fontSize: 13), textAlign: TextAlign.center),
          ],
        ),
      );
    }
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text('Nessun cliente trovato', style: TextStyle(color: Colors.grey[400], fontSize: 16)),
          ],
        ),
      );
    }
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final showDateHeader = index == 0 || _clientAppointments[index - 1]['data'] != appointment['data'];
        return Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            if (showDateHeader)
              Padding(
                padding: EdgeInsets.only(bottom: 6, top: index == 0 ? 0 : 12),
                child: Text(
